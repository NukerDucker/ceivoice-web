...
function clusterMerges(suggestions: SuggestedMerge[]): MergeGroup[] {
  ...
  const allChildIds = new Set(Array.from(groupMap.values()).flatMap((g) => g.childIds));
  return Array.from(groupMap.values()).filter((g) => !allChildIds.has(g.parentId));
}

// ─── Merge Group Card ─────────────────────────────────────────────────────────

function MergeGroupCard({ group, selected, onToggle }: { group: MergeGroup; selected: boolean; onToggle: () => void }) {
  return (
    <div
      onClick={onToggle}
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-gray-600">{1 + group.children.length} tickets to merge</span>
          {group.reason && (
      ...
      <div className="px-4 py-3">
        <div className="flex items-start gap-2 mb-2">
          <span className="shrink-0 mt-0.5 text-[9px] font-bold px-1.5 py-0.5 bg-violet-100 text-violet-600 rounded uppercase tracking-wide">Parent</span>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-800 leading-snug">{group.parent.title || '(Untitled)'}</p>
        ...
        {group.children.map((child) => (
          <div key={child.ticket_id} className="flex items-start gap-2 ml-6 mt-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-600 font-medium leading-snug">{child.title || '(Untitled)'}</p>
      ...
      <div className="px-4 py-2.5 bg-gray-50 border-t border-gray-100 flex items-center gap-2">
        ...
        <p className="text-[10px] text-gray-500">Requesters from merged tickets will become followers of #{group.parent.ticket_id}</p>
      </div>
    </div>
  );
}
...
function SuggestMergeModal({ onClose, onMergeComplete }: { onClose: () => void; onMergeComplete: () => void }) {
  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[calc(100vw-2rem)] sm:w-[520px] max-h-[80vh] bg-white rounded-2xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-red-400">
              ...
              <button onClick={fetchSuggestions} className="mt-4 text-xs text-violet-600 hover:underline">Try again</button>
            </div>
          ) : done ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-600">
              ...
              <button onClick={onClose} className="mt-5 px-4 py-2 bg-gray-900 text-white text-xs font-semibold rounded-xl hover:bg-gray-800 transition-colors">Close</button>
            </div>
          ) : groups.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <div className="flex flex-col gap-3">
              <p className="text-[11px] text-gray-400 mb-1">{groups.length} group{groups.length !== 1 ? 's' : ''} found · select which to merge</p>
              {groups.map((group) => (
                <MergeGroupCard key={group.parentId} group={group} selected={selectedGroups.has(group.parentId)} onToggle={() => toggleGroup(group.parentId)} />
              ))}
            </div>
          )}
        </div>
        ...
        {!loading && !error && !done && groups.length > 0 && (
          <div className="px-6 py-4 border-t border-gray-100 shrink-0">
            ...
            <div className="flex gap-3">
              ...
              >
                {merging ? <><Loader2 size={14} className="animate-spin" /> Merging…</> : <><Merge size={14} /> Merge {selectedCount} group{selectedCount !== 1 ? 's' : ''}</>}
              </button>
              <button onClick={onClose} disabled={merging} className="flex-1 border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition-colors text-sm">Cancel</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div className="fixed bottom-6 left-4 right-4 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 sm:w-auto z-50">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 bg-white px-5 py-4 rounded-2xl shadow-xl border border-gray-100">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            <span className="bg-gray-900 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">{selectedIds.length}</span>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {selectedIds.length > 3 && <span className="text-[11px] text-gray-400">+{selectedIds.length - 3} more</span>}
          </div>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="hidden sm:block w-px h-5 bg-gray-200" />
          <button onClick={onMerge} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
            <Merge size={15} /> Merge into Draft
          </button>
          <button onClick={onClear} className="flex items-center gap-1.5 text-gray-400 hover:text-gray-700 text-xs px-3 py-2 rounded-xl hover:bg-gray-100 transition-colors">
...
export default function AdminDraftQueuePage() {
  ...
  const handleCheck = (id: number, val: boolean) => {
    setSelectedIds((prev) => { const next = new Set(prev); if (val) next.add(id); else next.delete(id); return next; });
  };

  const handleSelectAll = () => {
    if (selectedIds.size === filtered.length) setSelectedIds(new Set());
    else setSelectedIds(new Set(filtered.map((t) => t.ticket_id)));
  };
  ...
  const handleConfirmMerge = async () => {
    ...
    setMergeError(null);
    try {
      await apiFetch(`/admin/${parentId}/merge`, { method: 'POST', body: JSON.stringify({ child_ticket_ids: childIds }) });
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-8 py-3 bg-gray-50 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer select-none">
              <input type="checkbox" checked={allChecked} onChange={handleSelectAll} className="w-4 h-4 rounded border-gray-300 accent-gray-900" />
            ...
            <div className="flex items-center gap-1.5">
              ...
              {!loading && (
                <span className="text-[10px] bg-violet-100 text-violet-600 font-bold px-1.5 py-0.5 rounded-full">{drafts.length}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-violet-200 bg-violet-50 hover:bg-violet-100 text-violet-600 text-xs font-semibold transition-colors whitespace-nowrap"
            >
              <Sparkles size={12} /> Suggest Merge
            </button>
            ...
            <div className="flex flex-col gap-2">
              {filtered.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                  ...
                  <p className="text-xs mt-1 opacity-60">{drafts.length === 0 ? 'No drafts pending review.' : 'Try adjusting your search.'}</p>
                </div>
              )}
            </div>
          )}
      ...
      <MergePopup selectedIds={Array.from(selectedIds)} onClear={handleClear} onMerge={handleMerge} />

      {showSuggestMerge && (
        <SuggestMergeModal onClose={() => setShowSuggestMerge(false)} onMergeComplete={fetchDrafts} />
      )}
      ...
      {showMergeConfirm && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8 w-[calc(100vw-2rem)] sm:w-[440px]">
                ...
                return (
                  <div key={id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {req?.email && <p className="text-[10px] text-gray-400 truncate">from {req.email}</p>}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex gap-3">
              <button onClick={handleConfirmMerge} disabled={merging} className="flex-1 bg-gray-900 hover:bg-gray-800 disabled:opacity-60 text-white font-semibold py-2.5 rounded-xl transition-colors text-sm">
                {merging ? 'Merging…' : 'Confirm Merge'}
              </button>
              <button onClick={() => setShowMergeConfirm(false)} disabled={merging} className="flex-1 border border-gray-200 text-gray-600 font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition-colors text-sm">
                Cancel
              </button>
            </div>
          </div>
