  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-4 sm:px-6 pb-8">
        ...
        {/* KPI Cards — 2 cols on all sizes, just tighter padding on mobile */}
        <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 sm:px-5 py-4 sm:py-5 flex flex-col gap-1">
            <div className="w-8 h-1.5 rounded-full mb-2" style={{ background: '#3B82F6' }} />
            <span className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">
              {activeTickets.length}
            </span>
            <span className="text-xs sm:text-sm font-semibold text-gray-700">Active Tickets</span>
        ...
        {/* Detail tables — stack on mobile, side-by-side on md+ */}
                  ...
                  return (
                    <div key={t.ticket_id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 border border-gray-100">
                      ...
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${
                        isSolved ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'
                      }`}>
